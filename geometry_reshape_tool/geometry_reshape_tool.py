...
class GeometryReshapeTool(QgsMapToolEmitPoint):
    ...
    def __init__(self, iface):
        ...
        locale_path = os.path.join(
            self.plugin_dir,
            'i18n',
            'GeometryReshapeTool_{}.qm'.format(locale))
        ...
        self.actions = []
        self.menu = self.tr(u'&Geometry Reshape Tool')
    ...
    def tr(self, message):
        ...
        return QCoreApplication.translate('GeometryReshapeTool', message)
    ...
    def initGui(self):
        ...
        icon_path = ':/plugins/geometry_reshape_tool/icon.png'
        self.iconAction=self.add_action(
            icon_path,
            text=self.tr(u'This plugin used for reshape existing geometry'),
            callback=self.run,
    ...
    def unload(self):
        ...
        for action in self.actions:
            self.iface.removePluginMenu(
                self.tr(u'&Geometry Reshape Tool'),
                action)
    ...
    # def vertex_add(self,geom, x, y, tol):
    #     p1, at, b1, after, d1 = geom.closestVertex(QgsPointXY(x, y))
    #     dist, p2, to,test = geom.closestSegmentWithContext(QgsPointXY(x, y))
    #     if at == 0:
    #         if dist < tol:
    ...
    def getNextId(self,geom,pt):
        if geom.wkbType() in [QgsWkbTypes.MultiPolygon]:
            vert_list=geom.asMultiPolygon()[0][0]
        else:
            vert_list=geom.asPolygon()[0]
        ...
        if event.button() == Qt.LeftButton:
            ...
            if len(self.coords) > 1:
                for feature in self.layer.selectedFeatures():
                    ...
                    # p1, at, b1, after, d1 = feat_geom.closestVertex(QgsPointXY(self.coords[0]))
                    # print('p1, at, b1, after, d1',p1, at, b1, after, d1)
                    #
                    # p2, at2, b2, after2, d2 = feat_geom.closestVertex(QgsPointXY(self.coords[-1]))
                    # print('p2, at2, b2, after2, d2', p2, at2, b2, after2, d2)
                    # gem=self.vertex_add(feat_geom,self.coords[0].x(),self.coords[0].y(),0.01)
                    # # self.layer.changeGeometry(feature.id(), gem)
                    # gem2 = self.vertex_add(feat_geom, self.coords[-1].x(), self.coords[-1].y(), tol=0.01)
                    # self.layer.changeGeometry(feature.id(), gem)
                    # con_lin=feat_geom.convertToType(QgsWkbTypes.LineGeometry)
                    # close1 = min([con_lin.closestSegmentWithContext(QgsPointXY(self.coords[0]))])
                    # print('close1',close1)
                    # ver=close1[1]
                    # idx1=close1[2]
                    # mapPt,layerPt1 = self.transformCoordinates(self.coords[0])
